...
#Preprocessing: Detecting gaps and types, Calculating ranges, relative volume and adding it into new columns 
```{r}
firstFile = TRUE
for (file in filesList) {
  ...
  for(i in 1:nrow(df)){
    ...
    dailyReturns <- append(dailyReturns, (df[i,6] - df[i,3]) / df[i,3])
    
    #calculating relative volume
    relativeVolume <- append(relativeVolume, df[i, 7]/volumeMean)
  ...
  #Combining dataframe of each stock into one dataframe
  if( firstFile){
    firstFile = FALSE
    stock_df <- df
  }
  else{
    stock_df <- rbind(stock_df, df)
  }
