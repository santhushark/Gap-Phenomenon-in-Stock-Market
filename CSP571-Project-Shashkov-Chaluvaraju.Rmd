...
# Create a sub frame
file <- file.choose()
#cleandf <- read_csv("/home/santosh/Desktop/MASTERS/STUDIES_MSDS/SEM_2_summer/Data_preparation_and_analysis/PROJECT/git_work/Gap-Phenomenon-in-Stock-Market/final_pre_processed_data/clean_preprocessed.csv", show_col_types = FALSE)
cleandf <- read_csv(file)
cols <- colnames(cleandf)
cols <- cols[- c(1,2,3,4,5,6,7,8,12)]
print(cols)
cols <- cols[-c(86:148)]
...
print(smry)
```
It is seen from the above summary p-values that, except for gap_size, month, month_lag.9, candle_body_metric_lag.9, return_lag.7, range_lag.6, month_lag.6, weekday_lag.5, month_lag.5, vol_change_lag.3, range_lag.3, weekday_lag.3 and weekday_lag.2, all other features have significant predicting power.
...
```
The mean squared error and residual mean squared error is almost the same for the trained model on both training and test dataset. Also an r-squared of 0.84 can be considered a good score in finance.
```{r}
#cleanup
rm(list=ls())
```
